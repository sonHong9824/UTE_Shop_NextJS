...
import { type Voucher, type UpdateVoucherData } from "@/types/Voucher";

interface VoucherEditModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (id: string, data: UpdateVoucherData) => Promise<void>;
    voucher: Voucher | null;
    loading?: boolean;
}

export const VoucherEditModal = ({ isOpen, onClose, onSubmit, voucher, loading = false }: VoucherEditModalProps) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (formData.type === "percentage" && formData.discountValue && formData.discountValue > 100) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const submitData: UpdateVoucherData = {
                ...formData,
                code: formData.code?.toUpperCase().trim(),
            };

            await onSubmit(voucher._id, submitData);
            onClose();
        } catch (error) {
                    ...
                    {voucher.usedCount && voucher.usedCount > 0 && (
                        <div className="bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 rounded-lg p-4">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <h4 className="font-semibold text-amber-900 dark:text-amber-100">Lưu ý</h4>
                                    <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                                        Voucher này đã được sử dụng {voucher.usedCount} lần.
                                        Việc thay đổi một số thông tin có thể ảnh hưởng đến những người dùng đã sử dụng voucher.
                                    </p>
                                </div>
                            </div>
                        </div>
